...
const GeneratePlan: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadPathways();
    // Fetch GitHub stars (unauthenticated)
    const fetchStars = async () => {
      try {
        const res = await fetch("https://api.github.com/repos/mohi-m/UIUC_Semester_Planner");
        if (!res.ok) return;
      ...
      <nav className="dashboard-nav">
        <div className="dashboard-nav-content">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <a
              href="https://github.com/mohi-m/UIUC_Semester_Planner"
              target="_blank"
              rel="noopener noreferrer"
              className="github-link"
              aria-label="View repository on GitHub"
              style={{ border: "1px solid", marginRight: 30 }}
            >
              {/* GitHub Mark */}
              ...
              <span className="github-text">uiuc-semester-planner</span>
              <span className="github-stars">⭐ {starCount ?? "—"}</span>
            </a>
